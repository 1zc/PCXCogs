...
class ReactChannel(commands.Cog):
    ...
    async def _save_channel(
        ...
        if isinstance(channel_type, list):
            try:
                ...
                return
        await self.config.custom("REACT_CHANNEL", ctx.guild.id, channel.id).set(
            {"channel_type": channel_type}
        )
    ...
    async def disable(self, ctx: commands.Context, channel: discord.TextChannel = None):
        ...
        await self.config.custom("REACT_CHANNEL", ctx.guild.id, channel.id).clear()
        await ctx.send(
            checkmark(
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
        ...
        if user.bot:
            return
        channel_type = await self.config.custom(
            "REACT_CHANNEL", payload.guild_id, payload.channel_id
        ).channel_type()
        try:
